...
public class BallScript : MonoBehaviour
{
    ...
    void HitTheBall()
    {
        if (BallScript.isThrowing)
        {
            ...
            {
                // Air Shot Function (there is a temp v3.up value)
                ...
                if (DirectShot.isDirectShot)
                {
                    ...
                    if (!_isDoneDirect)
                    {
                        instanceBallList[0].GetComponent<Rigidbody>().AddForce(Vector3.right * FindObjectOfType<PlayerControl>().hitPower);
                        Debug.Log("DirectShot is working");
                        _isDoneDirect = true;
                    }
                    if (PlayerControl.isPressed)
                    {
